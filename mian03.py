...
SELECTION_COLOR = (0, 120, 215) # 选中态背景色
...
TEXTS = {
    "CN": {
        ...
        "back": "返回",
        "save": "保存配置",
        "api_title": "LLM API 配置 (支持 Ctrl+A/V)",
...
class MockAIService:
    ...
    def query(self, text, config):
        """
        发送请求到大模型 API
        config: {"url": str, "key": str, "model": str}
        """
        api_key = config.get("key", "").strip()
        api_url = config.get("url", "").strip()
        model_name = config.get("model", "qwen-turbo").strip()

        # 1. 如果没有配置 Key，使用本地兜底
        if not api_key:
            return self._local_query(text, "【系统提示】未检测到 API Key，仅显示本地基础解释。\n请在“系统设置”中填入 Key 以获得完整 AI 体验。")
        ...
        if "chat/completions" not in api_url and "v1" in api_url:
        ...
        payload = {
            "model": model_name,
            "messages": [
                {"role": "system", "content": "你是一个精通数理统计的数据侦探助手。请用通俗易懂、带有悬疑风格的语言解释用户的统计学问题。着重解释用户提问的概念，字数尽量多一点，不超过500字。"},
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return f"API 返回格式异常: {str(res_json)}"
            else:
    ...
    def _local_query(self, text, prefix=""):
        key = text.upper().strip()
        content = ""
        found = False
        for k, v in self.fallback_kb.items():
            if k in key:
                content = v
                found = True
                break
        if not found:
            content = "本地数据库未找到该词条，且 API 未连接。"
        return f"{prefix}\n\n{content}" if prefix else content

ai_service = MockAIService()

# --- 【UI 组件重写】支持 Ctrl+A/V 的输入框 ---
class InputBox:
    ...
    def handle_event(self, event):
        ...
        if self.active:
            if event.type == pygame.KEYDOWN:
                ...
                # Ctrl + C (Pygame 自动处理剪贴板，这里无需手动，但为了逻辑完整)
    ...
    def draw(self, screen):
        ...
        # 绘制背景选中块
        text_w, text_h = font.size(display_text)
        text_x = self.rect.x + 10
        text_y = self.rect.centery - text_h // 2
        
        if self.selected and self.active:
            sel_rect = pygame.Rect(text_x, text_y, text_w, text_h)
...
class Game:
    def __init__(self):
        ...
        # API 设置
        self.api_config = {"url": "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions", "key": "", "model": "qwen-turbo"}
        ...
        return [
            # Ch1
            {
                ...
                "dialogue": [
                    ...
                    ("Detective", "别急着敲代码。先清洗数据，再建模，最后一定要看 summary 里的截距项。")
                ],
                "briefing": {
...
这就是为什么 Sigma 的策略会亏损——毫秒级的交易优势被繁重的挖矿程序拖慢了。而利用公司电力和硬件挖出的巨额比特币，从未进入公司账户，而是直接流向了一个加密的离岸钱包。这是一场“寄生虫”式的完美犯罪，既吸干了宿主，又掩盖了踪迹。""",
                "pool": [{"id": "corr", "text": "1. 计算相关矩阵", "type": "correct"}, {"id": "vif", "text": "2. 计算 VIF 值", "type": "correct"}, {"id": "drop", "text": "3. 剔除高 VIF 变量", "type": "correct"}, {"id": "refit", "text": "4. 重新拟合模型", "type": "correct"}, {"id": "check_alpha", "text": "检查截距项", "type": "wrong"}, {"id": "rf", "text": "随机森林", "type": "wrong"}],
            ...
            # Ch3
            {
                ...
                "briefing": {
                    ...
                    "steps": ["1. 提取 A/B 两组登录数据。", "2. 检查正态分布假设。", "3. 运行 T-Test。", "4. 解读 P-Value。"]
                },
...
'加班'？不，这是蓄意为之的入侵。B 就是那个在服务器植入后门的人。但一个初级分析师，哪来这么大的胆子？除非，他背后有更大的保护伞。""",
                "pool": [{"id": "group", "text": "1. 分组提取数据", "type": "correct"}, {"id": "norm", "text": "2. 检查正态性", "type": "correct"}, {"id": "ttest", "text": "3. 运行 T-Test", "type": "correct"}, {"id": "pvalue", "text": "4. 判断 P < 0.05", "type": "correct"}, {"id": "vif", "text": "计算 VIF", "type": "wrong"}, {"id": "ols", "text": "做线性回归", "type": "wrong"}],
                "correct_order": ["group", "norm", "ttest", "pvalue"],
            ...
            # Ch4
            {
                ...
                "briefing": {
                    ...
                    "steps": ["1. 绘制残差图 (Residual Plot)。", "2. BP 检验确认异方差。", "3. 对变量取 Log 对数。", "4. 重新回归并检查残差。"]
                },
...
B 崩溃了。他承认自己只是一个执行者。这笔巨额的比特币洗钱资金，最终流向了一个在开曼群岛的离岸账户，而该账户的实际控制人，指向了这栋大楼里最有权势的人——CEO。""",
                "pool": [{"id": "plot_res", "text": "1. 绘制残差图", "type": "correct"}, {"id": "bp_test", "text": "2. 进行 BP 检验", "type": "correct"}, {"id": "log", "text": "3. 对变量取 Log", "type": "correct"}, {"id": "refit", "text": "4. 重新回归", "type": "correct"}, {"id": "ttest", "text": "运行 T-Test", "type": "wrong"}, {"id": "alpha", "text": "检查 Alpha", "type": "wrong"}],
                "correct_order": ["plot_res", "bp_test", "log", "refit"],
            ...
            # Ch5
            {
                ...
                "briefing": {
                    ...
                    "steps": ["1. ADF 检验平稳性。", "2. 进行差分 (Differencing)。", "3. 观察 ACF/PACF 定阶。", "4. 建立 ARIMA 预测。"]
                },
...
案件结束。但数据背后的贪婪，似乎永远无法被模型拟合。""",
                "pool": [{"id": "adf", "text": "1. ADF 平稳性检验", "type": "correct"}, {"id": "diff", "text": "2. 差分处理", "type": "correct"}, {"id": "定阶", "text": "3. ACF/PACF 定阶", "type": "correct"}, {"id": "arima", "text": "4. ARIMA 预测", "type": "correct"}, {"id": "log", "text": "对变量取 Log", "type": "wrong"}, {"id": "ols", "text": "直接线性外推", "type": "wrong"}],
            ...
            for event in events:
                ...
                if self.show_ai_dialog:
                    ...
                    if res == "SUBMIT":
                        ...
                        if question.strip():
                            self.ai_answer_text = ai_service.query(question, self.api_config)
